...
class TestHostingService(TestCase):
    DATA_NO_CUSTOMER_ID = {'data': {'domainQuery': {'host': {'shopperId': '1234567'}}}}
    DATA_WITH_CUSTOMER_ID = {'data': {'domainQuery': {'host': {'customerId': '1234-5678-9012'}}}}
    ...
    def test_use_nes_product_false(self, suspend, get_use_nes):
        self._hosting_service.suspend('diablo', 'test-entitlementID', self.DATA_WITH_CUSTOMER_ID)
        suspend.assert_called_with(guid='test-entitlementID', data=self.DATA_WITH_CUSTOMER_ID)
    ...
    def test_use_nes_product_true_no_customer(self, suspend, get_use_nes, get_customer_id_from_shopper):
        self._hosting_service.suspend("diablo", "test-entitlementID", self.DATA_NO_CUSTOMER_ID)
