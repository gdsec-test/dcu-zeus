...
class TestNESHelper:
    ...
    @patch('zeus.events.suspension.nes_helper.NESHelper.poll_for_entitlement_status', return_value=True)
    def test_suspend_exception(self, post, poll_for_entitlement_status):
    ...
    @patch('zeus.events.suspension.nes_helper.NESHelper.poll_for_entitlement_status', return_value=True)
    def test_suspend_success(self, post, poll_for_entitlement_status):
    ...
    @patch('zeus.events.suspension.nes_helper.NESHelper.poll_for_entitlement_status', return_value=True)
    def test_suspend_fail(self, post, poll_for_entitlement_status):
        response = self._nes_helper.suspend('test-accountid', 'test-customerid')
    ...
    @patch('zeus.events.suspension.nes_helper.NESHelper.poll_for_entitlement_status', return_value='Entitlement error message')
    def test_suspend_entitlement_error(self, post, poll_for_entitlement_status):
    ...
    @patch('requests.post', side_effect=Timeout())
    @patch('zeus.events.suspension.nes_helper.NESHelper.poll_for_entitlement_status', return_value=True)
    def test_reinstate_exception(self, post, poll_for_entitlement_status):
        response = self._nes_helper.reinstate('test-accountid', 'test-customerid')
        assert_in('exception', response)

    @patch('requests.post', return_value=MagicMock(text='true', status_code=204))
    @patch('zeus.events.suspension.nes_helper.NESHelper.poll_for_entitlement_status', return_value=True)
    def test_reinstate_success(self, post, poll_for_entitlement_status):
        assert_true(self._nes_helper.reinstate('test-accountid', 'test-customerid'))

    @patch('requests.post', return_value=MagicMock(text='failure message', status_code=404))
    @patch('zeus.events.suspension.nes_helper.NESHelper.poll_for_entitlement_status', return_value=True)
    def test_reinstate_fail(self, post, poll_for_entitlement_status):
        response = self._nes_helper.reinstate('test-accountid', 'test-customerid')
        assert_in('failure message', response)
        assert_in('404', response)
    ...
    @patch('requests.post', return_value=MagicMock(status_code=204))
    @patch('zeus.events.suspension.nes_helper.NESHelper.poll_for_entitlement_status', return_value='Entitlement error message')
    def test_reinstate_entitlement_error(self, post, poll_for_entitlement_status):
        response = self._nes_helper.suspend('test-accountid', 'test-entitlementid')
        # Verify the response contains the entitlement error
        assert_equal('Entitlement error message', response)
    ...
    def test_entitlement_status_sucess(self, post):
        assert_true(self._nes_helper.poll_for_entitlement_status('test-accountid', 'test-customerid', 'SUSPENDED'))
    ...
    def test_entitlement_status_mismatch(self, post):
        ...
        # TODO LKM: figure out what to assert here