...
class RegisteredHandler(Handler):
    ...
    def intentionally_malicious(self, data):
        ...
        report_type = data.get('type')
        self._logger.info(f'In intentionally_malicious function.  data is {data}')
        shopper_id = self.shopper_api.get_shopper_id_from_dict(data)
        self._logger.info(f'get_shoper_id_from_dict returned {shopper_id}')
        shopper_id_list = self.shopper_api.get_list_of_ids_to_notify(data)
        self._logger.info(f'get_list_of_ids_to_notify returned {shopper_id_list}')
        source = data.get('source')
